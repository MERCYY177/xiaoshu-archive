...
function TreeNode({ page, tree, depth }: { page: NotionPageSummary; tree: Map<string | null, NotionPageSummary[]>; depth: number }) {
  ...
  return <div className="tree-branch"><Link href={`/p/${page.id}`} className="tree-node" style={{ paddingLeft: `${16 + Math.min(depth, 5) * 24}px` }}>{nested.length ? <Folder size={18} /> : <FileText size={18} />}<span>{page.icon && !page.icon.startsWith("http") ? `${page.icon} ` : ""}{page.title}</span><ChevronRight size={16} /></Link>{nested.map((child) => <TreeNode key={child.id} page={child} tree={tree} depth={depth + 1} />)}</div>;
}
